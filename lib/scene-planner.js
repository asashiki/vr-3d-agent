...
const ALLOWED_ACTIONS = ['Relax','Thinking','Surprised','Clapping','LookAround','Goodbye','Jump','StepForward','StepBack'];
...
function deterministicPlan({ text = '', scene = {}, errors = [] }) {
  ...
  if (/往前走|靠近我|到我.*这边|(?:你|mira|米拉).*过来|^过来|come\s+(closer|here)|step\s+forward/.test(words)) return normalizePlan({
    say: '好，我往你这边走一步。', emotion: 'happy', avatarAction: 'StepForward', commands: []
  });
  if (/往后走|退后|离我远|step\s+back|move\s+back/.test(words)) return normalizePlan({
    say: '好，我稍微退后一步。', emotion: 'neutral', avatarAction: 'StepBack', commands: []
  });
  if (/跳一下|跳跃|jump/.test(words)) return normalizePlan({
    say: '看好了。', emotion: 'happy', avatarAction: 'Jump', commands: []
  });

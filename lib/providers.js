...
let localStt = null;
function localSttWorker(env) {
  if (localStt) return localStt;
  const script = path.join(__dirname, '..', 'scripts', 'local-stt.py');
  const child = spawn(env.STT_PYTHON || 'python', [script, env.STT_MODEL || 'base'], { stdio: ['pipe', 'pipe', 'pipe'] });
  let buffer = '';
  const waiters = [];
  ...
  child.on('exit', () => { localStt = null; });
  localStt = {
    transcribe(file) {
      return new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error('STT_TIMEOUT')), Number(env.PROVIDER_TIMEOUT_MS || 40000));
        waiters.push((line) => {
          clearTimeout(timer);
          if (line.startsWith('ERROR:')) reject(new Error(line.slice(6) || 'STT_EMPTY_RESULT'));
          else resolve(line);
        });
...
function createLocalSttProvider(env) {
  return async ({ audio, fileName = 'speech.webm' }) => {
    const file = path.join(os.tmpdir(), `pocket-stt-${Date.now()}-${fileName.replace(/[^\w.-]+/g, '_')}`);
    fs.writeFileSync(file, audio);
    try {
      const text = String(await localSttWorker(env).transcribe(file) || '').trim();
...
function createCloudSttProvider(env, apiKey, fetchImpl) {
...
function createSTTProvider(env, fetchImpl = fetch) {
  ...
  if (configured === 'local') return createLocalSttProvider(env);
  if (configured === 'openai') {
    const apiKey = env.STT_API_KEY || env.OPENAI_API_KEY;
    return apiKey ? createCloudSttProvider(env, apiKey, fetchImpl) : null;
  }
  // auto/browser: do not send DeepSeek chat keys to OpenAI Whisper.
  const openaiLike = /openai\.com/i.test(env.OPENAI_BASE_URL || '');
  const apiKey = env.STT_API_KEY || (openaiLike ? env.OPENAI_API_KEY : '');
  if (apiKey && ['auto', 'browser'].includes(configured)) return createCloudSttProvider(env, apiKey, fetchImpl);
  if (configured === 'auto') return createLocalSttProvider(env);
  return null;
}

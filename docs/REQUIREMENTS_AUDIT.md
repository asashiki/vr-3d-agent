...
| M1 | Text, push-to-talk, TTS, lip sync, nine actions | `js/app.js`, `vrm-actor.js`, `actions.json`; six original actions plus Jump/StepForward/StepBack | Implemented; browser/Quest timing check pending |
| M2 | Movable/scalable tray and 25–35 licensed GLBs | `world-tray.js`, 30-item `ASSET_MANIFEST.json`, generator | Implemented; tray now starts hidden and offset; Quest ergonomics pending |
...
| M4 | Manual transform writes back; no hidden chain-of-thought | avatar/tray/object transform events and public tool events only | Implemented |
| M5 | Desktop fallback, immersive AR, controller and hand paths | `index.html`, `world-tray.js`, `QUEST_DEVICE_GUIDE.md` | Implemented in code; real Quest pending |
...
The software portion that can be built and tested in this environment is implemented and covered by automated checks, including the controller, avatar placement and server-STT remediation prompted by the first device trial. The project is **not allowed to claim final Quest completion** until the unchecked items in `QUEST_VALIDATION.md` have real-device screenshots, recording and performance evidence.
